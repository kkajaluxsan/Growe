...
export function hasAnyAiProvider() {
  ...
  return p.gemini || p.groq || p.openai;
}

/**
 * Provider order: optional AI_PROVIDER_ORDER=groq,gemini,openai
...
export function getAiProviderOrder() {
  ...
  if (custom) {
    const parsed = custom
      ...
      .filter((name) => name === 'gemini' || name === 'groq' || name === 'openai')
      .filter((name) => p[name]);
    if (parsed.length > 0) return parsed;
  }
  const preferGroq = process.env.AI_PREFER_GROQ === '1' || /^true$/i.test(process.env.AI_PREFER_GROQ || '');
  if (preferGroq) {
    return ['groq', 'openai', 'gemini'].filter((name) => p[name]);
  }
  return ['gemini', 'groq', 'openai'].filter((name) => p[name]);
}
