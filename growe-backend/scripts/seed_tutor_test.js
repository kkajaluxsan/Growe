...
async function seedTutor() {
  ...
  try {
    ...
    // 4. Create Availability for 3 days from now
    ...
    client.release();
    // pool.end(); // Don't end pool here if we might run multiple scripts, but fine for standalone
  }
}

seedTutor().then(() => process.exit(0)).catch((err) => {
    console.error(err);
    process.exit(1);
});
