...
export default function ShareButton({ title = 'Share', shareText, url, variant = 'secondary', size = 'sm', className = '' }) {
  ...
  const socialLinks = [
    {
      name: 'Twitter',
      href: `https://twitter.com/intent/tweet?text=${encodeUri(text)}&url=${encodeUri(fullUrl)}`,
      icon: (
    ...
    {
      name: 'WhatsApp',
      href: `https://wa.me/?text=${encodeUri(`${text} ${fullUrl}`)}`,
      icon: (
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      ...
      {open && (
        <div
          className="absolute right-0 mt-2 w-64 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-lg py-2 z-50"
          role="menu"
        >
          <a
            href={fullUrl}
            ...
            className="w-full flex items-center gap-2 px-4 py-2.5 text-left text-sm text-growe font-medium break-all hover:bg-slate-100 dark:hover:bg-slate-700"
            role="menuitem"
          >
          ...
          <div className="px-3 py-1.5 text-xs font-medium text-slate-500 dark:text-slate-400">Share on</div>
